...
ISR(TIMER1_OVF_vect)
{
    ...
    if (value != '\0') { // Data available from UART
        ...
        itoa(value, string, 10); // Convert to decimal string
        uart_puts("Decimal: ");
        uart_puts(string);
        uart_puts("\x1b[32m");
        uart_putc('\r'); // Send Carriage Return
        uart_putc('\n'); // Send Line Feed

        itoa(value, string, 16); // Convert to hexadecimal string
        uart_puts("Hexadecimal: 0x");
        uart_puts(string);
        uart_putc('\r'); // Send Carriage Return
        uart_putc('\n'); // Send Line Feed

        itoa(value, string, 2); // Convert to binary string
        uart_puts("Binary: 0b");
        uart_puts(string);
        uart_putc('\r'); // Send Carriage Return
        uart_putc('\n'); // Send Line Feed
